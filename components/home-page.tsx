    ...
    <>
      <section className={css({position: 'relative', minH: {base: '42rem', md: '47rem'}, display: 'flex', alignItems: 'end', overflow: 'hidden', color: 'paper', borderBottom: '1px solid var(--memorabilia-line)'})}>
        ...
        <div className={css({position: 'absolute', right: {base: 5, md: 9}, bottom: {base: 5, md: 8}, zIndex: 2, display: {base: 'none', sm: 'block'}, textAlign: 'right'})}>
          <span className={css({display: 'block', color: 'lime', fontSize: '10px', letterSpacing: '.15em', textTransform: 'uppercase', fontWeight: 800})}>{t('home.featured')}</span>
      ...
      <section id="about" className={css({mx: {base: 5, md: 9}, my: {base: 16, md: 24}, borderRadius: {base: '22px', md: '30px'}, overflow: 'hidden', bg: 'lime', color: 'night', p: {base: 6, md: 12}, position: 'relative'})}>
        <div className={css({position: 'absolute', right: '-3rem', top: '-6rem', w: '22rem', h: '22rem', border: '1px solid rgba(8,9,11,.18)', borderRadius: 'full', _after: {content: '""', position: 'absolute', inset: 12, border: '1px solid rgba(8,9,11,.15)', borderRadius: 'full'}})}/>
        <div className={css({position: 'relative', maxW: '55rem'})}>
          ...
          <p className={css({maxW: '48rem', mt: 6, fontSize: {base: 'md', md: 'lg'}, lineHeight: 1.65, color: 'rgba(8,9,11,.72)'})}>{t('home.archiveText')}</p>
          <div className={css({display: 'grid', gridTemplateColumns: {base: 'repeat(3,1fr)'}, gap: 3, mt: 10, pt: 8, borderTop: '1px solid rgba(8,9,11,.18)'})}>
